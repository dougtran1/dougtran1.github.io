// v2 Maximalist — scroll-pinned horizontal project scroll, infinite marquee, bold hero reveal.
...
mm.add(
  ...
  (context) => {
    ...
    if (track) {
      const trackWidth = track.scrollWidth / 2; // duplicated content, so half is one loop
      gsap.to(track, {
        x: -trackWidth,
        duration: 30,
        ease: "none",
    ...
    // Horizontal work pin + scrub (containerAnimation pattern from gsap-scrolltrigger skill).
...
// Resize handling — the pinned horizontal scroll + scrub is too fragile to recover
// gracefully from a mid-pin geometry change (refresh leaves the pin offset; kill +
// recreate leaves the cards translated off-screen). The pragmatic fix is to scroll
// back to the top and reload when the width changes meaningfully. Small height
// changes (mobile address bar) and trivial width nudges are ignored.
